...
type PosBodyProps = {
    mainForm: ReturnType<typeof useForm<PhysicalStockForm>>;
};




const PosBody = ({ mainForm: physicalStockForm }: PosBodyProps) => {
    // const physicalStockForm = useFormContext<PhysicalStockForm>();
    const { movementType } = usePos()
    const stockJournal = physicalStockForm.watch("stockJournal")
    ...
    useEffect(() => {
        ...
        const parentValue = physicalStockForm.getValues("stockJournal");
        if (parentValue && !isEqual(parentValue, stockJournalForm.getValues())) {
            stockJournalForm.reset(parentValue);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [physicalStockForm.watch("stockJournal")]);

    useEffect(() => {
        // when child changes, update parent
        const subscription = stockJournalForm.watch((value) => {
            const currentParent = physicalStockForm.getValues("stockJournal");
            if (!isEqual(currentParent, value)) {
                physicalStockForm.setValue("stockJournal", value as StockJournalForm, {
    ...
    }, [physicalStockForm, stockJournalForm]);

    if (physicalStockForm.watch("transactionLedger.id") === undefined || physicalStockForm.watch("partyLedger.id") === undefined) {
        return <StockJournalUnloadedView />
    }
    // console.log("PosBody Level: ", physicalStockForm.watch("stockJournal"), stockJournalForm.watch("stockJournalEntries"));
    return (
