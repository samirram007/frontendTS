...
import {
  voucherDispatchDetailSchema,
  type VoucherDispatchDetailForm,
...
const VoucherDispatchDetail02 = (props: VoucherDispatchDetailFormProps) => {
  ...
  const voucherDispatchForm = useForm<VoucherDispatchDetailForm>({
    resolver: zodResolver(
      voucherDispatchDetailSchema,
    ) as Resolver<VoucherDispatchDetailForm>,
  ...
  //     voucherDispatchForm.setValue('carrierName', transporter.name)
  //   }, [transporter])
  ...
  const handleOnClick = () => {
    ...
    const sanitizedData = {
      ...
      voucherId:
        dispatchData.voucherId === null
          ? undefined
          : dispatchData.voucherId?.toString(),
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      current.carrierName === transporter?.name
    ) {
      return
    }
    ...
    <Suspense fallback={<Loader className="animate-spin" />}>
      ...
      >
        <DialogTrigger asChild>
          <div
            className="border-2 rounded px-2 size-8 shadow-md flex items-center justify-center cursor-pointer select-none hover:bg-green-100 active:bg-green-200"
        ...
        <DialogContent className="sm:max-w-5xl">
          ...
          <div
            ref={dispatchRef}
            className="-mr-4 h-full w-full  overflow-y-auto py-1 pr-4"
          >
          ...
          <DialogFooter>
            <Button
              ...
              disabled={
                isPending ||
                voucherDispatchForm.watch('totalFare') === null ||
                voucherDispatchForm.watch('totalFare') === undefined ||
                voucherDispatchForm.watch('totalFare')! <= 0
              }
            >
...
const FreightCalculator = ({ form }: FreightCalculatorProps) => {
  ...
  useEffect(() => {
    ...
    if (Number(rate) > 0 && Number(weight) > 0) {
      const totalFare =
        Number(form.watch('rate')) * Number(form.watch('weight'))
  ...
  return (
    <div className="space-y-2 border-t-2 pt-2">
      <div className="text-center underline">
        Freight Calculator based on {freightBasis!}{' '}
      </div>
      <div className="text-sm italic text-gray-500">
        Freight calculator details can be added from the main Freight Details
        section.
      </div>
      <div className="hidden w-full  grid-cols-[1fr_2fr] gap-4 mb-2">
        <div className="text-left ">
          <div>Transporter</div>
          <div className="">
            <TransporterSelector name="carrierName" form={form} />
          </div>
        </div>
        <div className="text-left">
          <div>Vehicle Number</div>
          <div className="">
            <DeliveryVehicleRouteSelector name="motorVehicleNo" form={form} />
          </div>
        </div>
      </div>
