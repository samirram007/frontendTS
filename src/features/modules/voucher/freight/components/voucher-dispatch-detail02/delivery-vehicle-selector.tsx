...
export const DeliveryVehicleSelector = ({ form, name }: Props) => {
  ...
  const deliveryVehiclesFiltered = React.useMemo(() => {
    ...
    return deliveryVehicles.data?.filter(
      (deliveryVehicle: DeliveryVehicle) =>
        deliveryVehicle.transporter?.name === carrierName,
    )
  ...
  const handleSelect = (value: string) => {
    ...
    const selectedVehicle = deliveryVehiclesFiltered?.find(
      (deliveryVehicle: DeliveryVehicle) =>
        deliveryVehicle.vehicleNumber === value,
    )
    if (!selectedVehicle) {
      setOpen(true)
      return
  ...
  const frameworks = deliveryVehiclesFiltered?.map(
    (deliveryVehicle: DeliveryVehicle) => ({
