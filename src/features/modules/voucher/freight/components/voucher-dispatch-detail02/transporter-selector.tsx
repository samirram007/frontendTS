...
//   name: keyof DeliveryVehicleForm
// }

export const TransporterSelector = <TForm extends FieldValues>({
  form,
  name,
}: Props<TForm>) => {
  console.log('TransporterSelector rendered')
  const lastKeyRef = React.useRef<string | null>(null)

  const { setTransporter } = useGlobalContext()
  const [open, setOpen] = React.useState(false)

  console.log('TransporterSelector props:', { form, name })

  // const [value, setValue] = React.useState(form.getValues(name)?.toString())

  const value = React.useMemo(() => {
    return form.watch(name)?.toString()
  }, [form.watch(name)])
  ...
  // useEffect(() => {
  //   const v = form.getValues(name)?.toString() ?? ''
  //   setValue(v)
  // }, [form.watch(name)])
