...
export default function AuthLayout({ children }: Props) {
  return (
    <div className='min-h-svh lg:grid lg:grid-cols-[1fr_440px]'>
      ...
      {/* ── Left: Brand panel ─────────────────────────────────── */}
      <div className='relative hidden flex-col overflow-hidden bg-slate-950 text-white lg:flex'>
        ...
        <div className='relative z-10 flex items-center gap-3 p-8'>
          <div className='flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-500/10 ring-1 ring-cyan-500/30'>
            <IconFlask className='h-5 w-5 text-cyan-400' />
          </div>
          <div className='leading-none'>
            <p className='text-sm font-bold tracking-widest text-white'>PATHOCARE</p>
            <p className='text-[10px] tracking-widest text-slate-500 uppercase'>Laboratory Systems</p>
          </div>
        </div>
        ...
        <div className='relative z-10 flex flex-1 flex-col items-center justify-center px-10 text-center'>
          ...
          <div className='relative mb-10 flex items-center justify-center'>
            <div className='absolute h-52 w-52 rounded-full border border-white/[0.04]' />
            <div className='absolute h-36 w-36 rounded-full border border-white/[0.06]' />
            <div className='h-24 w-24 rounded-full border border-cyan-500/20 bg-cyan-500/5 flex items-center justify-center shadow-[0_0_40px_rgba(6,182,212,0.15)]'>
              <IconMicroscope className='h-10 w-10 text-cyan-400' />
            </div>
            {/* Orbiting accent dots */}
            <span className='absolute top-3 right-8 h-2 w-2 rounded-full bg-cyan-400/50' />
            <span className='absolute bottom-4 left-6 h-1.5 w-1.5 rounded-full bg-blue-400/50' />
            <span className='absolute top-1/2 -right-1 h-1 w-1 rounded-full bg-teal-400/60' />
          </div>
          ...
          <div className='mt-12 w-full max-w-xs'>
            ...
            <div className='mt-4 grid grid-cols-3 gap-2 text-center'>
              {['Sample Collection', 'Lab Analysis', 'Digital Report'].map((step, i) => (
                <div key={step}>
                  <div className='mx-auto mb-2 flex h-7 w-7 items-center justify-center rounded-full bg-slate-800 text-xs font-bold text-cyan-400 ring-1 ring-slate-700'>
                    {i + 1}
                  </div>
                  <p className='text-[10px] leading-tight text-slate-500'>{step}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Bottom footer */}
        <div className='relative z-10 flex items-center justify-between p-8 text-[11px] text-slate-600'>
          <span>© 2025 PathoCare Systems Pvt. Ltd.</span>
          <span className='rounded-full bg-slate-800 px-2 py-0.5 text-slate-400'>v2.5.0</span>
        </div>
      </div>

      {/* ── Right: Form panel ─────────────────────────────────── */}
      <div className='relative flex min-h-svh items-center justify-center bg-white p-8 dark:bg-slate-900 lg:min-h-0'>
