...
const VoucherTypeTablet = ({
  ...
  const colorClasses =
    VoucherTypeColorMapping.get(voucherTypeKey) ||
    'bg-amber-700/20 text-amber-700 dark:text-amber-400 border-amber-300'

  return (
    <Label
      className={cn("text-lg tablet rounded-2xl border-2 p-4 flex items-start " )}
      onClick={handleClick} 
    >
      <div className=" ">
        <Icon name={voucherType.icon} size={36} className={colorClasses} />{' '}
      </div>
      <div>
